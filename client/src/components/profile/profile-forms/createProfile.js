...
const CreateProfile = ({setAlert,createProfile,history,isAuthenticated,profile})=>{
    ...
    useEffect(()=>{
        const callApi = async ()=>{
            try{
                await scriptLoader("https://maps.googleapis.com/maps/api/js?key=AIzaSyDfJ2N7azyT5OST0Fy6nj2IRC0yAS_wWVc&libraries=places");
                setTimeout(()=>{
                    if(window.google){
                        autoComplete = new window.google.maps.places.Autocomplete(autoCompleteField.current,{"types": ['address']});
            ...
            <form >
                ...
                <div className="form-group optional-btn-wrapper">
                    <button type="button" className="btn btn-light" onClick={()=>{
                        onToggleSocialTab();
                    }}><i class="fa fa-expand" aria-hidden="true"></i> Add Social Link</button>
...
export default connect(mapStateToProps,mapDispatchAction) (withRouter(CreateProfile));
